...
function App() {
    ...
    return (
        <div className="flex flex-col items-center justify-center">
            <h1 className="text-3xl font-serif text-center mb-6 mt-4"> Coin Change Calculator</h1>
            <form onSubmit={handleSubmit}>
                <div className="mb-6">
                    <label className="block mb-2 text-lg font-semibold font-medium text-gray-900 dark:text-white">Target
                        Amount:</label>
                    <input type="number"
                ...
                <div className="mb-6">
                    <label className="block mb-2 text-lg font-semibold font-medium text-gray-900 dark:text-white">Select
                        Coin Denominators</label>
                    <div className="flex flex-wrap align-middle">
                        {ALLOWED_DENOMINATIONS.map(denominator => (
                            <div className="flex items-center mb-4 w-1/4 p-2" key={denominator}>
                                <input
                ...
                <div className="flex justify-center space-x-2">
                    <button type="submit"
                            className="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 w-32 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Calculate
                    </button>
                    <button type="reset"  onClick={handleReset}
                            className="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 w-32 dark:focus:ring-yellow-900">Reset
                    </button>
                </div>
            </form>
                ...
                <div className="mt-10">
                    ...
                    <ul className="flex justify-center space-x-4 mb-4">
                        {result.map((coin, index) => (
                            <li key={index} className="bg-gray-200 px-4 py-2 rounded-full">{coin}</li>
                        ))}
                    </ul>
                </div>
            )}
